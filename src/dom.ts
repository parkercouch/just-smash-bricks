...
export function updateLevelDisplay(currentLevel: number) {
  document.querySelector('.level')!.textContent = currentLevel.toString();
}
...
export function updateScore(combo: number) {
  ...
  document.querySelector('.score')!.textContent = SCORE.value.toString();
}

export function updateLives() {
  document.querySelector('.lives')!.textContent = `${LIVES.value - 1}`;
}
